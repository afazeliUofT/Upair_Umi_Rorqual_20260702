...
_VARIANT_FAMILY = {
    "main": "UPAIR Main",
    "shallow": "UPAIR Shallow",
    "deep": "UPAIR Deep",
    "narrow": "UPAIR Narrow",
    "wide": "UPAIR Wide",
    "wide_deep": "UPAIR Wide-deep",
    "mlpwide": "UPAIR MLP-wide",
}
...
def legend_label(method: str) -> str:
    """Return the full publication legend, including d, L, and r."""
    if method in _BENCHMARK_LEGEND:
        return _BENCHMARK_LEGEND[method]

    match = re.fullmatch(
        r"(wide_deep|mlpwide|shallow|narrow|deep|wide|main)_d(\d+)_b(\d+)_r(\d+)",
        method,
    )
    if match is None:
        return method

    family, d_model, num_blocks, mlp_ratio = match.groups()
    return (
        f"{_VARIANT_FAMILY[family]} "
        f"(d={d_model}, L={num_blocks}, r={mlp_ratio})"
    )
...
def timing_summary(data: pd.DataFrame) -> pd.DataFrame:
    ...
    for method, group in timed.groupby("method", sort=False):
        ...
        rows.append(
            {
                "method": method,
                "label": AXIS_SHORT.get(method, method),
...
def main() -> None:
    ...
    data["label"] = data["method"].map(AXIS_SHORT).fillna(data["method"])
    order = {name: index for index, name in enumerate(METHOD_ORDER)}
    ...
    legend_lines = [legend_label(method) for method in METHOD_ORDER]
    (out / "legend_labels.txt").write_text(
        "\n".join(legend_lines) + "\n",
        encoding="utf-8",
    )
    print("[INFO] Legend labels:")
    for label in legend_lines:
        print("  ", label)
